...
pub fn split_footer(file_slice: FileSlice) -> io::Result<(FileSlice, FileSlice)> {
    ...
    tantivy4java_debug!(
        "QUICKWIT DEBUG: [Thread {:?}] split_footer called with file_slice size: {}",
        thread_id,
        file_slice.len()
    );
    tantivy4java_debug!(
        "QUICKWIT DEBUG: [Thread {:?}] split_footer file_slice len: {}",
        thread_id,
        file_slice.len()
    );
    ...
    tantivy4java_debug!(
        "QUICKWIT DEBUG: [Thread {:?}] split_footer after split_from_end(4): body_and_footer_slice size: {}, footer_len_slice size: {}",
        thread_id,
        body_and_footer_slice.len(),
        footer_len_slice.len()
    );
    ...
    tantivy4java_debug!(
        "QUICKWIT DEBUG: [Thread {:?}] split_footer footer_len: {}",
        thread_id,
        footer_len
    );

    let (body_and_bundle_metadata, hotcache) =
        body_and_footer_slice.split_from_end(footer_len as usize);
    tantivy4java_debug!(
        "QUICKWIT DEBUG: [Thread {:?}] split_footer final result: body_and_bundle_metadata size: {}, hotcache size: {}",
        thread_id,
        body_and_bundle_metadata.len(),
        hotcache.len()
    );
...
impl BundleDirectory {
    ...
    pub fn open_split(split_file: FileSlice) -> io::Result<BundleDirectory> {
        tantivy4java_debug!(
            "QUICKWIT DEBUG: BundleDirectory::open_split - starting with file size: {}",
            split_file.len()
        );
        ...
        tantivy4java_debug!(
            "QUICKWIT DEBUG: split_footer succeeded, body size: {}",
            body_and_bundle_metadata.len()
        );
        ...
        match &result {
            ...
            Err(e) => {
                tantivy4java_debug!("QUICKWIT DEBUG: BundleDirectory::open_bundle failed: {}", e)
            }
        }
    ...
    pub fn open_bundle(file: FileSlice) -> anyhow::Result<BundleDirectory> {
        let thread_id = std::thread::current().id();
        tantivy4java_debug!(
            "QUICKWIT DEBUG: [Thread {:?}] BundleDirectory::open_bundle - opening file offsets with file size: {}",
            thread_id,
            file.len()
        );
        tantivy4java_debug!(
            "QUICKWIT DEBUG: [Thread {:?}] BundleDirectory::open_bundle - file slice len: {}",
            thread_id,
            file.len()
        );
        tantivy4java_debug!(
            "QUICKWIT DEBUG: [Thread {:?}] BundleDirectory::open_bundle - about to call BundleStorageFileOffsets::open with cloned file",
            thread_id
        );

        let cloned_file = file.clone();
        tantivy4java_debug!(
            "QUICKWIT DEBUG: [Thread {:?}] BundleDirectory::open_bundle - cloned file size: {}",
            thread_id,
            cloned_file.len()
        );
        ...
        tantivy4java_debug!(
            "QUICKWIT DEBUG: [Thread {:?}] BundleStorageFileOffsets::open succeeded",
            thread_id
        );
