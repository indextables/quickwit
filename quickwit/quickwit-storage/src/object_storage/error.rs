...
where
    E: std::error::Error + ToStorageErrorKind + Send + Sync + 'static,
{
        ...
        let source = if let SdkError::DispatchFailure(failure) = &error {
                ...
                && error_str.contains("failed to lookup address information")
            {
                ...
                if let Some(hostname) = extract_hostname_from_dispatch_failure(&failure) {
                    anyhow::anyhow!(
                        "dns error: failed to lookup address information for hostname '{}': {}",
                        hostname,
                        error_str
                    )
                } else {
